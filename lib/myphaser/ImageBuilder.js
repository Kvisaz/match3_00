...
ImageBuilder.cachedTexture = function (textureName) {
    if (this.game.cache.checkBitmapDataKey(generatedCacheName)) {
        return this.game.cache.getBitmapData(generatedCacheName);
    }
...
ImageBuilder.strokeRect = function (x, y, width, height, color, cachename, strokeWidth) {
    return this.game.add.image(x, y, this.strokeRectTexture(width, height, color, cachename, strokeWidth));
};